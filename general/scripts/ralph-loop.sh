...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  if echo "$OUTPUT" | grep -q "<promise>COMPLETE</promise>"; then
    ...
    RALPH_NOTIFY=true MESSAGE="Ralph completed all tasks! ✅ (iteration $i/$MAX_ITERATIONS)" "$PLUGIN_DIR/scripts/pushover-notification.sh"
    exit 0
  fi
...
RALPH_NOTIFY=true MESSAGE="Ralph reached max iterations ($MAX_ITERATIONS) ⚠️" "$PLUGIN_DIR/scripts/pushover-notification.sh"
exit 1
